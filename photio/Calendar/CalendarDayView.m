...
- (id)initWithFrame:(CGRect)_frame andDay:(NSString*)_day {
    CGRect dayFrame = CGRectMake(_frame.origin.x, _frame.origin.y + 0.35*_frame.size.height, 0.9*_frame.size.width, 0.6*_frame.size.height);
    self = [super initWithFrame:dayFrame];
    if (self) {
        self.textAlignment = UITextAlignmentRight;
        self.text = _day;
        self.font = [self.font fontWithSize:0.57*_frame.size.height];
